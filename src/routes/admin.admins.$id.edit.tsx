...
import { useAdminDetailsQuery, useUpdateAdminDetails } from "@/features/admins";
...
export const Route = createFileRoute("/admin/admins/$id/edit")({
  head: () => ({ meta: [{ title: "Edit admin — Dimisi" }] }),
...
function EditAdminPage() {
  ...
  if (isError || !user) {
    return (
      <EmptyState
        ...
        description={error?.message || "This administrator could not be found."}
        action={
          <Button asChild>
    ...
    updateMutation.mutate(
      ...
      },
      {
        onSuccess: () => {
          ...
          navigate({ to: "/admin/admins/$id", params: { id: userId } });
        },
        onError: (err) => {
  ...
  return (
    <>
      ...
      <PageHeader title={`Edit ${user.name}`} subtitle="Update profile, department, and access." />
      ...
      <form onSubmit={submit} className="glass max-w-2xl space-y-5 rounded-2xl p-6">
        ...
        <div className="flex justify-end gap-2 pt-2">
          <Button
            ...
            onClick={() => navigate({ to: "/admin/admins/$id", params: { id: userId } })}
            disabled={updateMutation.isPending}
          >
            Cancel
          </Button>
