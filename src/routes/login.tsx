...
import { signInWithGoogleOAuth, signInWithMicrosoftOAuth } from "@/lib/firebase";
...
function LoginPage() {
  ...
  const onMicrosoftSignIn = async () => {
    ...
    try {
      const { idToken } = await signInWithMicrosoftOAuth();
      const res = await authService.microsoftOAuthLogin({ idToken });
      const authUserData = res?.data?.user || res?.user;
      if (!authUserData) {
        throw new Error("Failed to receive profile information from server.");
      }
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Button
            ...
            disabled={googleSubmitting || microsoftSubmitting || submitting}
            className="h-11 w-full rounded-xl border-border/80 bg-background/50 font-medium transition-all hover:bg-accent hover:border-primary/40 text-xs sm:text-sm"
          >
            {googleSubmitting ? (
