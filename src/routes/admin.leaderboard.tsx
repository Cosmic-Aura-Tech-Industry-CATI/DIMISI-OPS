...
export const Route = createFileRoute("/admin/leaderboard")({
  head: () => ({
    meta: [
      { title: "Leaderboard — Poll" },
      { name: "description", content: "See who's leading the pack this month." },
      { property: "og:title", content: "Leaderboard — Poll" },
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {top3.map((e, i) => {
          ...
          return (
            ...
            >
              <div className={cn("absolute -top-10 left-1/2 h-24 w-24 -translate-x-1/2 rounded-full bg-gradient-to-br opacity-30 blur-2xl", podiumColor[i])} />
              <div className={cn("relative mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br text-white shadow-glow", podiumColor[i])}>
                <Icon className="h-6 w-6" />
              </div>
              <div className="relative mt-4 grid h-16 w-16 place-items-center mx-auto rounded-full bg-gradient-to-br from-primary/30 to-accent text-lg font-semibold">
                {e.avatar}
              </div>
              <h3 className="mt-3 font-display text-lg font-semibold">{e.name}</h3>
              <div className="mt-1"><IdBadge id={e.code} /></div>
        ...
        <ul className="divide-y divide-border/40">
          {rest.map((e) => (
            <li key={e.id} className="flex items-center gap-4 p-4 transition-colors hover:bg-muted/40">
              <div className="grid h-8 w-8 place-items-center rounded-lg bg-muted text-xs font-semibold">#{e.rank}</div>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 truncate text-xs text-muted-foreground"><IdBadge id={e.code} /> {e.department}</div>
              </div>
